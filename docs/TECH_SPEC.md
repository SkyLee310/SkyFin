...
│   └── check-client-bundle.mjs     # postbuild: fail on server-only secrets in client files
├── .env.example
├── vercel.json
└── package.json
```
...
- [ ] `lib/supabase/admin.ts` and `lib/ai/*` import `server-only`; the `postbuild` script (`scripts/check-client-bundle.mjs`) greps the client build and the service worker for `GEMINI`, `GOOGLE_SERVICE_ACCOUNT_KEY`, `BEGIN PRIVATE KEY`, `SUPABASE_SECRET_KEY`, `sb_secret_`, `VAPID_PRIVATE`, `CRON_SECRET`, and for the real secret values present at build time.
