...
export default function HistoryPage() {
  ...
  // Filters
  const [selectedMonth, setSelectedMonth] = useState(todayMYT().slice(0, 7)); // YYYY-MM
  const [selectedCategoryId, setSelectedCategoryId] = useState<string>("");
  const [selectedPayment, setSelectedPayment] = useState<PaymentMethod | "">("");
  const [selectedEssential, setSelectedEssential] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getTransactionsHistory({
        ...
        isEssential: selectedEssential !== null ? selectedEssential : undefined,
      }),
    ]).then(([catRes, groups]) => {
      if (isMounted) {
  ...
  }, [selectedMonth, selectedCategoryId, selectedPayment, selectedEssential, refreshTrigger]);
      ...
      <div className="flex flex-wrap gap-2 py-3 border-b border-slate-100">
        ...
        <select
          ...
          onChange={(e) => {
            ...
            if (val === "needs") setSelectedEssential(true);
            else if (val === "wants") setSelectedEssential(false);
            else setSelectedEssential(null);
          }}
          className="text-xs bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-slate-700 font-medium focus:outline-none focus:ring-1 focus:ring-emerald-500 min-h-[36px]"
        >
